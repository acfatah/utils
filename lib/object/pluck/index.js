...
function pluckMultipleKeys(arrayOfObjects, keys, strict) {
  return arrayOfObjects.reduce((accumulator, current, index) => {
    const results = []

    if (strict && !isKeysIterable(current))
      throw new Error(`Invalid object supplied at index ${index}.`)

    keys.forEach((key, index) => {
      if (strict && !Object.keys(current).includes(key))
        throw new Error(`Object without key "${key}" supplied at index ${index}.`)
...
function pluckSingleKey(arrayOfObjects, key, strict) {
  return arrayOfObjects.reduce((accumulator, current, index) => {
    if (strict && !isKeysIterable(current))
      throw new Error(`Invalid object supplied at index ${index}.`)

    if (strict && !current[key])
      throw new Error(`Object without key "${key}" supplied at index ${index}.`)
